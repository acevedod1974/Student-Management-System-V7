...
interface CourseStore {
  ...
  addStudent: (courseId: string, student: Omit<Student, 'id' | 'grades' | 'finalGrade'>) => void;
  deleteStudent: (courseId: string, studentId: string) => void;
  updateGrade: (courseId: string, studentId: string, gradeId: string, newScore: number) => void;
  updateStudent: (courseId: string, studentId: string, updates: Partial<Omit<Student, 'id' | 'grades' | 'finalGrade'>>) => void;
  updateExamName: (courseId: string, examIndex: number, newName: string) => void;
...
export const useCourseStore = create<CourseStore>()(
  persist(
    (set, get) => ({
      ...
      addStudent: (courseId, studentData) => set((state) => {
      ...
      updateGrade: (courseId, studentId, gradeId, newScore) => set((state) => ({
        courses: state.courses.map((course) =>
          course.id === courseId
            ? {
                ...course,
      ...
      updateStudent: (courseId, studentId, updates) => set((state) => ({
        courses: state.courses.map((course) =>
          course.id === courseId
            ? {
                ...course,
      ...
      updateExamName: (courseId, examIndex, newName) => set((state) => ({
        courses: state.courses.map((course) =>
          course.id === courseId
            ? {
                ...course,
      ...
      addExam: (courseId, examName) => set((state) => ({
        courses: state.courses.map((course) =>
          course.id === courseId
            ? {
                ...course,
      ...
      deleteExam: (courseId, examIndex) => set((state) => ({
        courses: state.courses.map((course) =>
          course.id === courseId
            ? {
                ...course,
